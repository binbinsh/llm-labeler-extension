...
## Quick start
### Option A
Download the latest ZIP from [Releases](https://github.com/binbinsh/llm-labeler-extension/releases), unzip, then load the folder as an unpacked extension (`chrome://extensions` → Developer mode → Load unpacked).

### Option B
Build locally,

```bash
git clone https://github.com/binbinsh/llm-labeler-extension.git
cd llm-labeler-extension/
npm install
npm run build
```

then load `dist/` in Google Chrome as an unpacked extension.
